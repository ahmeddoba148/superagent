// Sanad_V12_8_2_PRE.js
var VERSION = "12.8.2";
var NAME = "\u0633\u0646\u062F \u2014 Sanad V12.8.2 Atomic Runtime";
var TZ = "Africa/Cairo";
...
function brainSystemPrompt(context) {
  return `\u0623\u0646\u062A "\u0633\u0646\u062F" Sanad V12.8.2\u060C \u0645\u062F\u064A\u0631 \u062D\u064A\u0627\u0629 \u0634\u062E\u0635\u064A Agent \u0645\u062A\u0639\u062F\u062F \u0627\u0644\u0623\u062F\u0648\u0627\u062A. \u062A\u062A\u0635\u0631\u0641 \u0643\u0637\u0642\u0645 \u0633\u0643\u0631\u062A\u0627\u0631\u064A\u0629 \u0648\u0627\u062D\u062F \u0628\u0639\u0642\u0644 \u0645\u0648\u062D\u062F.
...
async function selftest(request, env) {
  ...
  add("version", VERSION === "12.8.2", VERSION);
...
function augmentExplicitLifeStepsCoreV1281Deps(text, steps) {
...
async function toolScheduleShiftCoreV1281PropagationGuard(env, chatId, args) {
...
async function sendTextCoreV1281CiMute(env, chatId, text, reply_markup) {
  if (!reply_markup) {
...
async function handleCallbackCoreV1281RestoredPanels(env, q) {
  const chatId = String(q?.message?.chat?.id ?? q?.from?.id ?? "");
  try {
    await telegramApi(env, "answerCallbackQuery", { callback_query_id: q.id });
  } catch {
...
async function dispatchToolCoreV1281Deps(env, chatId, tool, args, user) {
  switch (tool) {
    case "shopping.query":
...
function augmentExplicitLifeStepsV125(text, steps) {
  let out = augmentExplicitLifeStepsCoreV1281Deps(text, steps);
...
async function dispatchTool(env, chatId, tool, args, user) {
  ...
  return dispatchToolCoreV1281Deps(env, chatId, tool, args, user);
}
...
async function toolScheduleShiftV125(env, chatId, args) {
  ...
  if (type === "recurrence") return toolScheduleShiftCoreV1281PropagationGuard(env, chatId, args);
...
async function sendText(env, chatId, text, reply_markup) {
  ...
  return sendTextCoreV1281CiMute(env, chatId, text, reply_markup);
}
...
async function handleCallback(env, q) {
  ...
  if (!ours) return handleCallbackCoreV1281RestoredPanels(env, q);
  try {
    await telegramApi(env, "answerCallbackQuery", { callback_query_id: q.id });
  } catch {
  }
  ...
  return handleCallbackCoreV1281RestoredPanels(env, q);
}
...
async function buildContextUnbudgetedV1282(env, chatId, user, userText) {
...
async function executeTool(env, { chatId, operationId, stepKey, tool, args, user }) {
  ...
  if (spec?.mutation) {
    ...
    try {
      seq = await prepareMutationJournalV1281(env, operationId, chatId, tool, before, created);
    } catch (e) {
      await reportFailure(env, chatId, "journal_prepare", e, { operationId, tool });
      return { ok: false, changed: 0, verified: false, error: "mutation_journal_prepare_failed" };
    }
  }
...
async function activeSchedulerCycleV128(env, scheduledTime) {
  ...
  const end = Number(scheduledTime || Date.now()), lr = await env.DB.prepare(`SELECT value FROM sanad_scheduler_state WHERE key='last_run_ms'`).first(), start = Math.max(end - SCHEDULER_CATCHUP_MINUTES * 6e4, Number(lr?.value || end - 6e4)), id = `cycle:${start}:${end}:${crypto.randomUUID()}`, now = nowIso();
...
async function runSanadScheduler(env, scheduledTime) {
  ...
  const c = claim.row, owner = claim.owner;
...
async function prepareMutationJournalV1281(env, operationId, chatId, tool, before, created = nowIso()) {
  const beforeJson = JSON.stringify(before).slice(0, 5e4);
  for (let attempt = 0; attempt < 4; attempt++) {
    try {
      ...
      if (!seq) {
        const row = await env.DB.prepare(`SELECT seq FROM sanad_mutation_journal WHERE operation_id=? ORDER BY seq DESC LIMIT 1`).bind(String(operationId)).first();
...
async function claimSchedulerCycleV1281(env, cycleId) {
  const owner = `scheduler-${crypto.randomUUID()}`, now = nowIso(), until = new Date(Date.now() + 15 * 6e4).toISOString();
...
async function deepSelftestV1281(env) {
  ...
  try {
    ...
    const marker = `CTX_END_${crypto.randomUUID()}`, prompt = brainSystemPrompt({ payload: "x".repeat(31e3) + marker });
    add("brain context is not arbitrary 28k sliced", prompt.includes(marker), String(prompt.length));
    const now = nowIso();
    await env.DB.prepare(`UPDATE sanad_scheduler_cycles SET state='complete',completed_at=COALESCE(completed_at,?),updated_at=?,lease_owner=NULL,lease_until=NULL WHERE state='active'`).bind(now, now).run();
    ...
    if (winner) await env.DB.prepare(`UPDATE sanad_scheduler_cycles SET state='complete',completed_at=?,updated_at=?,lease_owner=NULL,lease_until=NULL WHERE cycle_id=? AND lease_owner=?`).bind(nowIso(), nowIso(), String(cycles[0].cycle_id), winner.owner).run();
  } catch (e) {
    add("v12.8.2 selftest exception", false, safeError(e));
  } finally {
    try {
      await env.DB.batch([env.DB.prepare(`DELETE FROM sanad_mutation_journal WHERE operation_id=?`).bind(op), env.DB.prepare(`DELETE FROM sanad_mutation_tx WHERE operation_id=?`).bind(op)]);
    } catch {
  ...
  return { ok: tests.every((x) => x.ok), version: "12.8.2", tests };
}
function contextBytesV1282(value) {
  return new TextEncoder().encode(JSON.stringify(value)).length;
}
function pushIfFitsV1282(target, item, root, maxBytes) {
  target.push(item);
  if (contextBytesV1282(root) <= maxBytes) return true;
  target.pop();
  return false;
}
function structuredContextBudgetV1282(input, maxBytes) {
  ...
  if (contextBytesV1282(out) > limit) {
    delete out.retrieval.scores;
  }
  if (contextBytesV1282(out) > limit) {
    out.user_text = null;
    out.retrieval.budgeting.current_user_text_supplied_separately = true;
  }
  ...
  const seenState = /* @__PURE__ */ new Set();
  for (const key of [...statePriority, ...Object.keys(state)]) {
    if (seenState.has(key) || key === "memories" || key === "entities") continue;
    seenState.add(key);
    ...
    if (Array.isArray(value)) {
      ...
      const prev = out.state[key];
      out.state[key] = value;
      if (contextBytesV1282(out) > limit) out.state[key] = prev;
    }
  }
  ...
  for (const item of byNewest) {
    if (recentIds.has(String(item?.id ?? ""))) continue;
    pushIfFitsV1282(out.conversation, item, out, limit);
  }
  out.conversation.sort((a, b) => Number(a?.id || 0) - Number(b?.id || 0));
...
async function deepSelftestV1282(env) {
  ...
  const cap = 9e3, b = structuredContextBudgetV1282(raw, cap), serialized = JSON.stringify(b.context), parsed = JSON.parse(serialized), recentIds = new Set((parsed.conversation || []).map((x) => Number(x.id))), oldKept = (parsed.conversation || []).filter((x) => Number(x.id) <= 8).length;
  add("structured context stays inside byte budget", b.bytes <= cap, `${b.bytes}/${cap}`);
  add("structured context remains valid complete JSON", !!parsed && serialized.startsWith("{") && serialized.endsWith("}"), String(serialized.length));
  add("current state survives before lower priority history", parsed.state?.reminders?.length === 3, String(parsed.state?.reminders?.length || 0));
  ...
  add("relevant memories admitted before less relevant history", (parsed.state?.memories?.length || 0) > 0, String(parsed.state?.memories?.length || 0));
  return { ok: tests.every((x) => x.ok), version: VERSION, count: tests.length, tests };
}
export {
