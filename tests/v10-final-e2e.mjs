...
try{
 ...
 let sends=telegram.filter(x=>x.method==='sendMessage');ok('welcome sent',sends.length===1&&sends[0].body.text.includes('Super Agent'),JSON.stringify(sends));ok('welcome no buttons',!sends[0].body.reply_markup,JSON.stringify(sends[0].body));
