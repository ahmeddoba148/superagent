/* SuperAgent V11.6.1 CLEAN — VS Code zero-Problems workspace profile; full JS validation retained; suggestion-only diagnostics disabled at workspace level. */
/* SuperAgent V11.6.1 FINAL — zero VS Code/TypeScript diagnostics hardening over certified V11.5.2; runtime behavior and 3-model chain retained. */
/* SuperAgent V11.6.1 FINAL — fixed Google chain; V11.5.1 hardening retained; durable inbox lease-collision self-recovery, bounded contention wait, and strict direct-chat deadline; certified staging matrix. */
/* SuperAgent V11.6.1 FINAL — Google-only Lite-priority chain; stronger live-proven third fallback; V11.5 hardening retained; no AI router. */
/* SuperAgent V11.6.1 FULL — hardening over V11.4; Google Lite-priority 3-model chain; no AI router. */
/* SuperAgent V11.4 FULL — Google-only stable 3-model chain: Gemini 3.5 Flash-Lite primary, Gemini 3.1 Flash-Lite fallback 1, Gemini 3 Flash Preview fallback 2. V11.3 reliability hardening retained; AI router remains removed. */
/* SuperAgent V11.3 FULL — one-batch reliability, safety and latency hardening over certified V11.2; simple 3-model chain retained, AI router remains removed. */
/* SuperAgent V11.2 FULL — router removed. Exactly 3 AI models: GPT OSS 20B primary, Gemini 3.5 Flash-Lite fallback 1, Qwen 3.6 27B fallback 2. Existing deterministic engines, verifier/rollback guards, shopping To-Do, durable Telegram inbox, reminders, recurrence, voice, memory, Life OS and V10.7.1 feature engine are retained. */
/* SuperAgent V11.1 FIX — fast natural chat + semantic task router.
...
/* SuperAgent V11.1 FULL — V10.7.1 full feature engine + V11.2 simple three-model failover + latency/UX hardening.
Key V11.1 changes: 12s bounded AI budget, simple sequential primary/fallback execution, shorter durable queue lease/retry sleeps, deterministic multiline shopping, interactive shopping to-do checklist after mutations. */
/* SuperAgent V11 FULL — built from the complete V10.7.1 feature engine.
V11 upgrades: adaptive model ranking from D1 stats, deterministic identity, explicit shopping entity-preservation validation. Existing Cloudflare bindings and mature V10.7.1 feature/reliability engine are retained. */
const V10_VERSION="11.6.1";const V10_NAME="سوبر إيجنت V11.6.1 — Gemini Lite ثلاثي + Hardening شامل للاعتمادية والأمان والسرعة";const RELIABILITY_RECEIPT_TTL_MINUTES=10;const RUNTIME_FAILURE_RETENTION_DAYS=30;const VOICE_MAX_BYTES=25*1024*1024;const V10_AUDIT_LIMIT=100;const V10_WORLD_CONTEXT_LIMIT=50;const V10_MAX_SHOP_ITEMS=80;const TIME_ZONE="Africa/Cairo";const PENDING_TTL_MINUTES=30;const CONFLICT_TTL_MINUTES=20;const TOTAL_AI_BUDGET_MS=10000;const V112_CHAT_TOTAL_BUDGET_MS=7600;const V112_CHAT_TIMEOUT_MS=2700;const AI_RATE_LIMIT_PER_MINUTE=15;const CONVERSATION_MEMORY_LIMIT=24;const CONTEXT_DAYS=365;const CONTEXT_NEAR_LIMIT=160;const CONTEXT_RELEVANT_LIMIT=80;const CONTEXT_RULE_LIMIT=120;const FREE_PERIOD_HORIZON_DAYS=180;const FREE_SLOT_HORIZON_DAYS=60;const CONFLICT_LOOKAHEAD_DAYS=60;const MAX_RULE_OCCURRENCES=5000;const SCHEDULER_MAX_CATCHUP_MINUTES=1440;const MAX_ADVANCE_ALERT_MINUTES=10080;const SHOW_MODEL_USED_TO_ADMIN=false;const LIVE_NEWS_MAX=8;const LIVE_NEWS_TTL_MINUTES=5;const LIVE_WATCH_BATCH_SIZE=8;const PRAYER_CACHE_TTL_MINUTES=720;const HOLIDAY_CACHE_TTL_MINUTES=720;const DEFAULT_COUNTRY_CODE="EG";const DEFAULT_CITY="Cairo";const DEFAULT_COUNTRY="Egypt";const OMNIAI_INTERNAL_URL="https://omniai-engine.ahmeddoba91.workers.dev/v1/chat/completions";const MODEL_CHAIN=[
...
{short:"G3.1-L",name:"Gemini 3.1 Flash-Lite",id:"gemini::gemini-3.1-flash-lite",timeoutMs:3600,role:"fallback_1",tags:["chat","shopping","schedule","json","arabic"]},
...
await ensureRuntimeSchemaReadyV115(env);
...
const url=new URL(request.url);
const key=adminKeyFromRequestV115(request);
...
let persistWorldUpdatesSafely=async function persistWorldUpdatesSafely(env,chatId,intent){try{await persistWorldUpdatesFromIntent(env,chatId,intent);}catch(e){await recordRuntimeFailure(env,{chatId,scope:"world_model_noncritical",error:e,context:{action:intent?.action}});}}
...
async function reliabilityDiagnostics(request,env){
  const url=new URL(request.url);const key=adminKeyFromRequestV115(request);
  if(!env.SETUP_KEY||key!==env.SETUP_KEY)return json({ok:false,error:"غير مصرح"},401);
  try{
...
const V106_LEASE_RETRY_COUNT=16;
const V106_LEASE_RETRY_DELAY_MS=650;
...
let triggerDrainContinuationV106=async function triggerDrainContinuationV106(env,chatId,origin){
...
let drainTelegramInboxV106=async function drainTelegramInboxV106(env,chatId,origin=''){
  if(!env?.DB)return;
  ...
  if(!acquired){if(origin&&await hasRunnableInboxV106(env,chatId)){const delay=Math.min(1800,Math.max(700,Math.floor(V106_INBOX_LEASE_MS/20)))+Math.floor(Math.random()*120);await sleepV106(delay);await triggerDrainContinuationV106(env,chatId,origin);}return;}
  try{
...
let drainPendingTelegramInboxV106=async function drainPendingTelegramInboxV106(env){
  if(!env?.DB)return;
...
let handleV11Identity=async function handleV11Identity(text,env,chatId){
...
const v113ChatKey=telegramChatKeyV105(update);v113SetUpdateContext(v113ChatKey,claim.key);
if(claim.key&&await v113ReplayCommittedUpdateIfAny(env,v113ChatKey,claim.key)){await finishTelegramUpdate(env,claim.key,"done",null);v113ClearUpdateContext(v113ChatKey);return;}
...
try{
if(!isPublicMode(env)&&!isAllowedUserV113(env,chatId)){
...
if(compoundResult)await sendShoppingResultAndChecklistV111(env,chatId,compoundResult);
return;
}
...
v113ClearUpdateContext(v113ChatKey);
...
let executeShoppingPlanV107=async function executeShoppingPlanV107(env,chatId,intent){
  const plan=normalizeShoppingPlanV107(intent.shopping);if(plan.mode==='query'){const answer=await answerShoppingQueryV107(env,chatId,plan);await sendText(env,chatId,answer);await saveConversationMessage(env,chatId,'assistant',answer);await showShoppingList(env,chatId,null,{startSession:false});return}
...
const V112_CHAT_MODELS=MODEL_CHAIN;
...
let v112LooksLikeToolOrStateRequest=function v112LooksLikeToolOrStateRequest(text){
...
let v112LooksLikeDirectChat=function v112LooksLikeDirectChat(text){
  const raw=String(text||"").trim();
  if(!raw)return false;
  return !v112LooksLikeToolOrStateRequest(raw);
}
let v112CallPlainChat=async function v112CallPlainChat(env,model,text,history=[],timeoutCapMs=V112_CHAT_TIMEOUT_MS){
...
let tryV112FastChat=async function tryV112FastChat(env,chatId,text,history=[]){
...
let handleDirectCommands=async function handleDirectCommands(env,chatId,text){
...
let executeIntent=async function executeIntent(env,chatId,intent,options={}){
...
const result=await applyBulkShift(env,chatId,plan);if(Number(result?.changed||0)>0)intent._v113_operation_committed=true;
...
const result=await bulkDeleteSchedule(env,chatId,intent);if(Number(result?.oneTime||0)+Number(result?.rulesDeleted||0)+Number(result?.ruleExceptions||0)>0)intent._v113_operation_committed=true;
...
.run();
await writeAudit(env,chatId,{action:"update",entityType:"schedule_rule",entityId:String(id),summary:`تعديل تكرار: ${next.title}`,before:current,after:next,undo:{type:"restore_schedule_rule",row:current}});intent._v113_operation_committed=true;
...
await writeAudit(env,chatId,{action:"update",entityType:"reminder",entityId:String(id),summary:`تعديل موعد: ${next.title}`,before:current,after:next,undo:{type:"restore_reminder_snapshot",row:current}});intent._v113_operation_committed=true;
...
intent._v113_operation_committed=true;
answer+=modelFooter(env,chatId,intent);
await sendText(env,chatId,answer,quickMenuKeyboard());
await saveConversationMessage(env,chatId,"assistant",answer);
}

let handleCallbackQuery=async function handleCallbackQuery(query,env){
...
let cancelReminder=async function cancelReminder(
env,
chatId,
id,
intent=null
...
let deleteScheduleRule=async function deleteScheduleRule(env,chatId,id,intent=null){
...
let parseIntentWithFallback=async function parseIntentWithFallback(env,userText,validationContext){
...
throw Object.assign(new Error("V11.4: موديلات Gemini الثلاثة لم تُرجع خطة موثوقة، فمغيّرتش أي بيانات. جاري إعادة المحاولة تلقائيًا عند فشل مؤقت."),{v112_failures:failures});
}

async function parseIntentWithFallbackLegacy(env,userText,validationContext,V11_MODEL_POOL){
const REMINDER_MODELS=Array.isArray(V11_MODEL_POOL)&&V11_MODEL_POOL.length?V11_MODEL_POOL:MODEL_CHAIN;
const systemPrompt=
...
let callOneModel=async function callOneModel(
env,
model,
systemPrompt,
userText,
...
let forgetUserMemory=async function forgetUserMemory(
env,
chatId,
query
){
...
let handleLifeDirectCommands=async function handleLifeDirectCommands(
env,
chatId,
text
){
...
let fetchPrayerDay=async function fetchPrayerDay(
env,
profile,
date,
force=false
...
let fetchPublicHolidays=async function fetchPublicHolidays(
env,
profile,
year
){
...
let buildLiveRealityContext=async function buildLiveRealityContext(
env,
chatId,
text,
profile,
...
let fetchGdeltNews=async function fetchGdeltNews(
query,
max=LIVE_NEWS_MAX
){
const q=
...
let callTextModel=async function callTextModel(
env,
prompt,
timeoutMs=3500
){
...
let answerChatWithLiveData=async function answerChatWithLiveData(
env,
chatId,
userText,
intent,
...
let sendText=async function sendText(
env,
chatId,
text,
replyMarkup=null
...
let editOrSend=async function editOrSend(
env,
chatId,
messageId,
text,
...
let telegramApi=async function telegramApi(
env,
method,
payload
...
let telegramApiWithRetry=async function telegramApiWithRetry(
env,
method,
payload,
maxRetries=1
...
async function undoLastAuditAction(env,chatId){
  ...
  try{
    if(u.type==="delete_created_schedule"){
      ...
      const x=u.row||{};
      await env.DB.prepare(`INSERT OR REPLACE INTO smart_list_items(id,list_id,chat_id,title,normalized_title,quantity,status,position,created_at,updated_at,meta_json) VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
        .bind(Number(x.id),Number(x.list_id),chatId,x.title,x.normalized_title,x.quantity,x.status,Number(x.position||0),x.created_at||new Date().toISOString(),new Date().toISOString(),String(x.meta_json||"{}")).run();
...
async function sendShoppingResultAndChecklistV111(env,chatId,result){
...
let toggleShoppingItemCallback=async function toggleShoppingItemCallback(env,chatId,messageId,itemId){
...
let finishShoppingSessionCallback=async function finishShoppingSessionCallback(env,chatId,messageId,listId){
...
let markShoppingByText=async function markShoppingByText(env,chatId,text,status){
...
let removeShoppingByText=async function removeShoppingByText(env,chatId,text){
...
let snapshotV102ShoppingMutation=async function snapshotV102ShoppingMutation(env,chatId,items){
...
let rollbackV102ShoppingMutation=async function rollbackV102ShoppingMutation(env,chatId,snapshot,result){
...
let restoreShoppingListSnapshotV1034=async function restoreShoppingListSnapshotV1034(env,chatId,snapshot){
...
let tryDirectShoppingDeleteV1034=async function tryDirectShoppingDeleteV1034(env,chatId,raw){
...
let clearEverythingV105=async function clearEverythingV105(env,chatId){
  const before={
...
async function tryDirectMixedTimedShoppingV105(env,chatId,raw){
  ...
  const r=await addShoppingItems(env,chatId,mix.items);await sendShoppingResultAndChecklistV111(env,chatId,r);return true;
}
...
let handleV10DirectCommands=async function handleV10DirectCommands(env,chatId,text,{fromVoice=false}={}){
  ...
  m=raw.match(/^(?:امسح|شيل|احذف)\s+(.+?)\s+(?:من\s+المشتريات|من\s+قائمة\s+المشتريات)$/iu);if(m){const r=await removeShoppingByText(env,chatId,m[1]);if(r.ambiguous?.length){await sendText(env,chatId,`لقيت أكتر من صنف ممكن يكون المقصود: ${r.ambiguous[0].names.join("، ")}. حدده أكتر.`);return true;}await sendText(env,chatId,r.changed?`🗑️ شلت ${r.changed} من قائمة المشتريات.`:"ملقتش الصنف ده في المشتريات.");if(r.changed)await showShoppingList(env,chatId,null,{startSession:false});return true;}
...
let transcribeTelegramVoice=async function transcribeTelegramVoice(env,voice){
...
let runV10PeriodicIntelligence=async function runV10PeriodicIntelligence(env,scheduledTime){
...
let selfTestEndpoint=async function selfTestEndpoint(request,env){const url=new URL(request.url);const key=adminKeyFromRequestV115(request);if(!env.SETUP_KEY||key!==env.SETUP_KEY)return json({ok:false,error:"غير مصرح"},401);return json({version:V10_VERSION,...runV10SelfTests()});}
...
/* ========================= SUPERAGENT V11.3 HARDENING LAYER =========================
   Safety/latency/reliability patch over V11.2. Keeps the simple 3-model chain and no AI router.
   The model may propose intent; deterministic code decides whether state can change and verifies it.
   ================================================================================ */
const V113_EXTERNAL_TIMEOUT_MS=2200;
const V113_TELEGRAM_TIMEOUT_MS=4500;
const V113_LIVE_TOTAL_BUDGET_MS=5200;
const V113_RECOVERY_CHAT_BATCH=6;
const V113_ACTIVE_UPDATE_CONTEXT=new Map();

function allowedChatIdsV113(env){
  const vals=[env?.ADMIN_CHAT_ID,env?.ALLOWED_CHAT_ID,env?.ALLOWED_CHAT_IDS]
    .filter(v=>v!=null&&String(v).trim())
    .flatMap(v=>String(v).split(/[\s,;|]+/))
    .map(v=>v.trim()).filter(Boolean);
  return new Set(vals);
}
function isAllowedUserV113(env,chatId){
  if(isPublicMode(env))return true;
  return allowedChatIdsV113(env).has(String(chatId));
}

let needsCurrentExternalDataV113=function needsCurrentExternalDataV113(text){
  ...
  return /(?:سعر|اسعار|صرف|الدولار|اليورو|الريال|الذهب|الدهب|بورصه|بورصة|سهم|اسهم|الطقس|الجو|درجه الحراره|درجة الحرارة|حراره اليوم|حرارة اليوم|نتيجه المباراه|نتيجة المباراة|الماتش|الاسكور|ترتيب الدوري|متاح دلوقتي|مفتوح دلوقتي|حاليا|حاليًا|دلوقتي).{0,35}(?:كام|ايه|اي|اليوم|النهارده|الآن|دلوقتي|حاليا|حاليًا|وصل|عامل)|(?:كام|ايه|اي).{0,20}(?:سعر|صرف|الدولار|اليورو|الذهب|الدهب|الجو|الطقس|درجه الحراره|درجة الحرارة)/u.test(t);
}
function needsHolidayContextV113(text){return /(?:اجازه رسميه|اجازة رسمية|العطله الرسميه|العطلة الرسمية|شم النسيم|عيد العمال|عيد تحرير|تحرير سيناء|ثوره|ثورة|راس السنه|رأس السنة|الكريسماس|عيد الميلاد)/u.test(normalizeArabicLoose(String(text||'')));}
function needsPersonalStateContextV113(text){
  const t=normalizeArabicLoose(String(text||''));
  return /(?:فاكر|فاكرني|ذاكرتك|ذاكرتي|قلتلك|قولتلك|اسمي|اسم مراتي|اسم زوجتي|مراتي|زوجتي|ابني|ابنى|بنتي|اخويا|اختي|مواعيدي|جدولي|التذكيرات بتاعتي|المشتريات عندي|قائمتي|عندي ايه|عندي اي|فين الموعد|امتى موعدي|انت عارف عني|تعرف عني)/u.test(t);
}
function looksLikeMutationTextV113(text){
  const t=normalizeArabicLoose(String(text||''));
  return /^(?:ضيف|زود|حط|سجل|احذف|امسح|شيل|الغ|الغي|عدل|غير|غيّر|خلي|خلى|اجل|أجل|قدم|قدّم|فكرني|فكرنى|ذكرني|ذكرنى|نبهني|نبهنى|وقف|شغل|استأنف|استكمل|سنووز|snooze|جبت|اشتريت|خدت|ملقتش|سيب|افتكر|انسى|انسي)/u.test(t)
    || /(?:للمشتريات|في المشتريات|فى المشتريات|المتكرر|المتكرره|المتكررة)/u.test(t)
    || /(?:عاوز|عايز|محتاج).{0,30}(?:اشتري|اشترى|اجيب|أجيب|جيب|هات)/u.test(t);
}
function looksLikeShoppingMutationV113(text){
  const t=normalizeArabicLoose(String(text||''));
  if(/^(?:احذف|امسح|شيل|فضي|افرغ)\s+(?:(?:كل|قائمه|قايمه|قائمة|قايمة)\s+)?(?:المشتريات|قائمه\s+المشتريات|قايمه\s+المشتريات|قائمة\s+المشتريات|قايمة\s+المشتريات)(?:\s+كلها)?$/u.test(t))return false;
  return /(?:للمشتريات|في المشتريات|فى المشتريات|قائمة المشتريات|قائمه المشتريات)/u.test(t)
    || /^(?:جبت|اشتريت|خدت|ملقتش|مش موجود|مفيش|سيب|سيبلي|مش هاجيب)\s+/u.test(t)
    || /(?:عاوز|عايز|محتاج).{0,30}(?:اشتري|اشترى|اجيب|أجيب|جيب|هات)/u.test(t);
}

// Narrow direct-chat gate: only context-free/stable chat may bypass the full contextual agent.
v112LooksLikeToolOrStateRequest=function(text){
  const raw=String(text||'').trim();
  const t=v112NormalizeCasual(raw);
  if(!t)return false;
  if(raw.startsWith('/'))return true;
  if(needsCurrentExternalDataV113(raw)||needsPrayerContext(raw)||needsPersonalStateContextV113(raw))return true;
  if(looksLikeCreateRequest(raw)||looksLikeMutationTextV113(raw))return true;
  return /(?:مشتريات|قائمه الشراء|قائمة الشراء|تذكير|ميعاد|موعد|سنووز|snooze|تابعلي|راقبلي|وقف متابعه|وقف متابعة|الوقت عندي|انا فين|ذاكره|ذاكرة|نموذج العالم|المواعيد|التكرارات|قائمة المشتريات|قائمه المشتريات)/u.test(t);
};
v112LooksLikeDirectChat=function(text){const raw=String(text||'').trim();return !!raw&&!v112LooksLikeToolOrStateRequest(raw);};

function basicRealityContextV113(profile){
  const p=profile||{timezone:TIME_ZONE,city:DEFAULT_CITY,country:DEFAULT_COUNTRY,country_code:DEFAULT_COUNTRY_CODE};
  const now=zonedNow(p.timezone||TIME_ZONE);
  const line=`الوقت الحي: ${now.date} ${now.time}:${now.second} — ${now.weekday} — ${now.timezone}\nالمكان: ${p.city||DEFAULT_CITY}, ${p.country||DEFAULT_COUNTRY} (${p.country_code||DEFAULT_COUNTRY_CODE})`;
  return{now,profile:p,prayers:{today:null,tomorrow:null},holidays:[],occasion:'',text:line,short:`🕒 ${now.weekday} ${now.date} — ${now.time}\n📍 ${p.city||DEFAULT_CITY} · ${now.timezone}`};
}
const __V112_buildLiveRealityContext=buildLiveRealityContext;
buildLiveRealityContext=async function(env,chatId,text,profile,force=false){
  profile=profile||await getUserProfile(env,chatId);
  if(force||needsPrayerContext(text)||needsHolidayContextV113(text))return __V112_buildLiveRealityContext(env,chatId,text,profile,force);
  return basicRealityContextV113(profile);
};

async function cacheGetAnyV113(env,key){
  try{const r=await env.DB.prepare(`SELECT value_json FROM live_cache WHERE cache_key=? LIMIT 1`).bind(key).first();return r?parseJsonObject(r.value_json):null;}catch{return null;}
}
async function fetchWithTimeoutV113(url,options={},timeoutMs=V113_EXTERNAL_TIMEOUT_MS){
  const controller=new AbortController();
  const parent=options?.signal;
  let onAbort=null;
  if(parent){if(parent.aborted)controller.abort();else{onAbort=()=>controller.abort();parent.addEventListener('abort',onAbort,{once:true});}}
  const timer=setTimeout(()=>controller.abort(),Math.max(250,Number(timeoutMs)||V113_EXTERNAL_TIMEOUT_MS));
  try{return await fetch(url,{...options,signal:controller.signal});}
  finally{clearTimeout(timer);if(parent&&onAbort)try{parent.removeEventListener('abort',onAbort);}catch{}}
}

fetchPrayerDay=async function(env,profile,date,force=false){
  const method=prayerMethodForProfile(profile);
  const loc=profile.latitude!=null&&profile.longitude!=null?`${Number(profile.latitude).toFixed(5)},${Number(profile.longitude).toFixed(5)}`:`${profile.city},${profile.country}`;
  const key=`prayer:${loc}:${date}:m${method}`;
  if(!force){const c=await cacheGet(env,key);if(c)return c;}
  const d=toAladhanDate(date);
  const url=profile.latitude!=null&&profile.longitude!=null
    ?`https://api.aladhan.com/v1/timings/${encodeURIComponent(d)}?latitude=${encodeURIComponent(profile.latitude)}&longitude=${encodeURIComponent(profile.longitude)}&method=${method}`
    :`https://api.aladhan.com/v1/timingsByAddress/${encodeURIComponent(d)}?address=${encodeURIComponent(`${profile.city}, ${profile.country}`)}&method=${method}`;
  try{
    const r=await fetchWithTimeoutV113(url,{headers:{accept:'application/json'}},V113_EXTERNAL_TIMEOUT_MS);
    if(!r.ok)throw new Error(`Prayer API HTTP ${r.status}`);
    const j=await r.json(),data=j?.data;if(!data?.timings)throw new Error('Prayer API data missing');
    const timings={};for(const k of ['Fajr','Sunrise','Dhuhr','Asr','Maghrib','Isha'])timings[k]=cleanPrayerTime(data.timings[k]);
    const out={date,timings,hijri:data?.date?.hijri||null,timezone:String(data?.meta?.timezone||profile.timezone||TIME_ZONE),method:Number(data?.meta?.method?.id??method)};
    return cachePut(env,key,out,PRAYER_CACHE_TTL_MINUTES);
  }catch(e){const stale=await cacheGetAnyV113(env,key);if(stale)return stale;throw e;}
};
fetchPublicHolidays=async function(env,profile,year){
  const cc=String(profile.country_code||DEFAULT_COUNTRY_CODE).toUpperCase();const key=`holidays:${cc}:${year}`;
  const c=await cacheGet(env,key);if(c)return Array.isArray(c.items)?c.items:[];
  let items=[];
  for(const url of[`https://date.nager.at/api/v4/Holidays/${encodeURIComponent(cc)}/${year}`,`https://date.nager.at/api/v3/PublicHolidays/${year}/${encodeURIComponent(cc)}`]){
    try{const r=await fetchWithTimeoutV113(url,{headers:{accept:'application/json'}},1800);if(!r.ok)continue;const j=await r.json();if(Array.isArray(j)){items=j.map(x=>({date:String(x.date||''),name:String(x.localName||x.name||''),english:String(x.name||'')})).filter(x=>validDate(x.date));if(items.length)break;}}catch{}
  }
  if(!items.length){const stale=await cacheGetAnyV113(env,key);if(Array.isArray(stale?.items))return stale.items;}
  await cachePut(env,key,{items},HOLIDAY_CACHE_TTL_MINUTES);return items;
};
fetchGdeltNews=async function(query,max=LIVE_NEWS_MAX){
  const q=String(query||'').trim();if(!q)return[];
  try{const url=`https://api.gdeltproject.org/api/v2/doc/doc?query=${encodeURIComponent(q)}&mode=artlist&maxrecords=${clamp(Number(max||8),1,20)}&format=json&sort=datedesc&timespan=24h`;
    const r=await fetchWithTimeoutV113(url,{headers:{accept:'application/json'}},2200);if(!r.ok)return[];const j=await r.json();const arr=Array.isArray(j?.articles)?j.articles:Array.isArray(j)?j:[];
    return arr.map(x=>({title:String(x.title||''),url:String(x.url||''),domain:String(x.domain||''),seen:String(x.seendate||x.datetime||'')})).filter(x=>x.title&&x.url).slice(0,max);
  }catch{return[];}
};

callTextModel=async function(env,prompt,timeoutMs=3500){
  const deadline=Date.now()+Math.min(Math.max(700,Number(timeoutMs)||3500),V113_LIVE_TOTAL_BUDGET_MS);
  const failures=[];
  for(const model of MODEL_CHAIN){
    const remaining=deadline-Date.now();if(remaining<350)break;
    const controller=new AbortController();const ms=Math.max(300,Math.min(remaining,Number(model.timeoutMs||1500)));const timer=setTimeout(()=>controller.abort(),ms);
    try{const req=new Request(OMNIAI_INTERNAL_URL,{method:'POST',headers:{Authorization:`Bearer ${env.OMNIAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:model.id,messages:[{role:'system',content:'أجب بالنص المطلوب فقط وباختصار.'},{role:'user',content:prompt}],max_tokens:700,stream:false}),signal:controller.signal});
      const res=await env.OMNIAI_SERVICE.fetch(req);const raw=await res.text();let j;try{j=JSON.parse(raw)}catch{j=null};const text=v11ExtractModelText(j||{}).trim();if(res.ok&&text)return text;failures.push(`${model.id}:${res.status}`);
    }catch(e){failures.push(`${model.id}:${safeError(e)}`);}finally{clearTimeout(timer);}
  }
  throw new Error(`تعذر الحصول على رد حي من الموديلات (${failures.length} محاولات).`);
};
answerChatWithLiveData=async function(env,chatId,userText,intent,reality,history,memories){
  if(!needsLiveNews(userText)&&String(intent?.live_kind||'')!=='news'){
    return 'المعلومة دي لحظية ومفيش مصدر مباشر موصول لها حاليًا، فمش هخمن رقم أو حالة قديمة.';
  }
  const started=Date.now();const q=String(intent?.live_query_en||userText||'').trim();
  const news=await fetchGdeltNews(q,LIVE_NEWS_MAX);
  if(!news.length)return 'مش قادر أتحقق من مصدر الأخبار الحي دلوقتي، فمش هخمن معلومة حديثة. جرّب تاني بعد شوية.';
  const feed=news.map((x,i)=>`${i+1}. ${x.title} | ${x.domain} | ${x.seen} | ${x.url}`).join('\n');
  const remaining=Math.max(500,V113_LIVE_TOTAL_BUDGET_MS-(Date.now()-started));
  const prompt=`الطلب: ${userText}\n\nالوقت/المكان:\n${reality?.text||''}\n\nنتائج أخبار حية تم جلبها الآن:\n${feed}\n\nأجب بالمصري باختصار ودقة. استخدم النتائج فقط للأخبار الحالية. لو النتائج لا تكفي قل ذلك. لا تخترع.`;
  return callTextModel(env,prompt,remaining);
};

// Force current-information questions through the live safety path even when a model forgets the flag.
const __V112_parseIntentWithFallback=parseIntentWithFallback;
parseIntentWithFallback=async function(env,userText,validationContext){
  const intent=await __V112_parseIntentWithFallback(env,userText,validationContext);
  const base=String(validationContext?.baseText||'');
  if(intent?.action==='chat'&&needsCurrentExternalDataV113(base)){intent.needs_live_data=true;intent.live_kind=needsLiveNews(base)?'news':'general';}
  return intent;
};

// Provider-adaptive planner JSON: try structured JSON, then retry same model without response_format when unsupported.
callOneModel=async function(env,model,systemPrompt,userText,timeoutMs){
  const deadline=Date.now()+Math.max(450,Number(timeoutMs)||1500);
  let structured=true,lastErr=null;
  for(let pass=0;pass<2;pass++){
    ...
    try{
      const body={model:model.id,messages:[{role:'system',content:systemPrompt},{role:'user',content:userText}],max_tokens:1200,stream:false};if(structured)body.response_format={type:'json_object'};
      const req=new Request(OMNIAI_INTERNAL_URL,{method:'POST',headers:{Authorization:`Bearer ${env.OMNIAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify(body),signal:controller.signal});
      const res=await env.OMNIAI_SERVICE.fetch(req);const raw=await res.text();let data;try{data=JSON.parse(raw)}catch{throw new Error(`HTTP ${res.status}: OmniAI رجّع رد غير JSON`);}
      if(!res.ok){const msg=String(data?.error?.message||data?.message||`OmniAI HTTP ${res.status}`);if(structured&&res.status>=400&&res.status<500&&/(response.?format|json.?object|unsupported|not supported|schema)/i.test(msg)){structured=false;lastErr=new Error(msg);continue;}throw new Error(msg);}
      const content=v11ExtractModelText(data).trim();if(!content)throw new Error('الموديل رجّع رد فاضي');return parseModelJson(content);
    }catch(e){if(e?.name==='AbortError'){const x=new Error(`Timeout بعد ${Math.max(0,deadline-Date.now())}ms`);x.name='AbortError';throw x;}lastErr=e;if(!structured)break;throw e;}finally{clearTimeout(timer);}
  }
  throw lastErr||new Error('فشل JSON من الموديل');
};

// Shopping postcondition verifier. It checks the requested semantics, not merely that "something changed".
function activeShopRowsV113(items){return (items||[]).filter(x=>['pending','bought','unavailable','skipped'].includes(String(x.status)));}
function shopRowsByTargetV113(items,target){
  const rows=activeShopRowsV113(items);if(target==='__all__')return rows;if(target==='__last__')return rows.length?[rows[rows.length-1]]:[];if(target==='__first__')return rows.length?[rows[0]]:[];
  const ranked=rows.map(r=>({r,s:scoreShopTargetV107(target,r)})).filter(x=>x.s>0).sort((a,b)=>b.s-a.s);if(!ranked.length)return[];const top=ranked[0].s;return ranked.filter(x=>x.s===top).map(x=>x.r);
}
function numEqualV113(a,b){return Number.isFinite(Number(a))&&Number.isFinite(Number(b))&&Math.abs(Number(a)-Number(b))<1e-9;}
function verifyShoppingPlanV113(before,after,plan){
  const errors=[];const b=activeShopRowsV113(before?.items),a=activeShopRowsV113(after?.items);const afterById=new Map(a.map(x=>[Number(x.id),x]));
  for(const op of plan.operations||[]){
    if(op.op==='reorder'){errors.push('reorder غير مدعوم بأمان');continue;}
    if(op.op==='add'){
      const key=shopVariantKeyV107(op.title,op.meta||{});const hits=a.filter(r=>shopVariantKeyV107(r.title,parseShopMetaV107(r.meta_json))===key);
      if(!hits.length){errors.push(`الإضافة لم تُثبت: ${op.title}`);continue;}const m=parseShopMetaV107(hits[hits.length-1].meta_json);
      if(op.quantity_value!=null&&!numEqualV113(m.quantity_value,op.quantity_value)&&!b.some(r=>shopVariantKeyV107(r.title,parseShopMetaV107(r.meta_json))===key)){errors.push(`كمية الإضافة غير متطابقة: ${op.title}`);}
      continue;
    }
    const targets=shopRowsByTargetV113(b,op.target);if(!targets.length){errors.push(`الهدف غير موجود قبل التنفيذ: ${op.target}`);continue;}
    if(op.op==='remove'){for(const r of targets)if(afterById.has(Number(r.id)))errors.push(`الحذف لم يثبت: ${r.title}`);continue;}
    for(const r of targets){const x=afterById.get(Number(r.id));if(!x){errors.push(`العنصر اختفى بدون طلب حذف: ${r.title}`);continue;}const bm=parseShopMetaV107(r.meta_json),am=parseShopMetaV107(x.meta_json);
      if(op.op==='mark_bought'&&x.status!=='bought')errors.push(`الحالة ليست bought: ${r.title}`);
      if(op.op==='mark_pending'&&x.status!=='pending')errors.push(`الحالة ليست pending: ${r.title}`);
      if(op.op==='replace'&&canonicalShoppingKeyV105(x.title)!==canonicalShoppingKeyV105(op.replacement))errors.push(`الاستبدال لم يثبت: ${r.title}`);
      if(op.op==='set_meta')for(const [k,v] of Object.entries(op.meta||{})){if(v!==''&&v!=null&&JSON.stringify(am[k]??null)!==JSON.stringify(v))errors.push(`meta.${k} غير متطابق: ${r.title}`);}
      if(op.op==='set_quantity'&&op.quantity_value!=null&&!numEqualV113(am.quantity_value,op.quantity_value))errors.push(`الكمية غير متطابقة: ${r.title}`);
      if(op.op==='increment'&&op.quantity_value!=null){const base=bm.quantity_value==null?1:Number(bm.quantity_value);if(!numEqualV113(am.quantity_value,base+Number(op.quantity_value)))errors.push(`الزيادة غير متطابقة: ${r.title}`);}
      if(op.op==='multiply'&&op.factor!=null){if(bm.quantity_value==null||!numEqualV113(am.quantity_value,Number(bm.quantity_value)*Number(op.factor)))errors.push(`الضرب غير متطابق: ${r.title}`);}
    }
  }
  return{ok:errors.length===0,errors};
}
executeShoppingPlanV107=async function(env,chatId,intent){
  const plan=normalizeShoppingPlanV107(intent.shopping);
  if(plan.mode==='query'){const answer=await answerShoppingQueryV107(env,chatId,plan);await sendText(env,chatId,answer);await saveConversationMessage(env,chatId,'assistant',answer);await showShoppingList(env,chatId,null,{startSession:false});return;}
  if(plan.operations.some(x=>x.op==='reorder'))throw new Error('إعادة ترتيب المشتريات غير مدعومة بأمان في V11.3.');
  const before=await snapshotShoppingV107(env,chatId);let changed=0;
  try{
    for(const op of plan.operations)changed+=await applyShopOperationV107(env,chatId,op);
    const after=await snapshotShoppingV107(env,chatId);
    const verified=verifyShoppingPlanV113(before,after,plan);if(!verified.ok)throw new Error(`V11.3 shopping verifier: ${verified.errors.join(' | ')}`);
    const same=JSON.stringify(before.items)===JSON.stringify(after.items);if(same&&plan.operations.length)throw new Error('خطة المشتريات لم تنتج تغييرًا قابلًا للتحقق');
    await writeAudit(env,chatId,{action:'shopping_v113',entityType:'shopping_transaction',entityId:String(after.list?.id||''),summary:`تنفيذ ${plan.operations.length} عملية مشتريات موثقة`,before,after,undo:{type:'restore_shopping_snapshot_v107',snapshot:before},strict:true});
    intent._v113_operation_committed=true;
  }catch(e){await restoreShoppingSnapshotV107(env,chatId,before);throw e;}
  const list=await getDefaultShoppingList(env,chatId,false),rows=list?await getShoppingItems(env,chatId,list.id):[],pending=rows.filter(x=>x.status==='pending');
  const answer=`✅ تم تنفيذ طلب المشتريات والتحقق منه. ${changed?`التغييرات: ${changed}. `:''}المتبقي في القائمة: ${pending.length}.`;
  v113ArmMutationReceipt(chatId,'shopping');await sendText(env,chatId,answer);await saveConversationMessage(env,chatId,'assistant',answer);await showShoppingList(env,chatId,null,{startSession:false});
};

function resolveShoppingTextTargetsV113(rows,text){
  const active=activeShopRowsV113(rows),wanted=splitShoppingItems(text).filter(Boolean);const resolved=[],ambiguous=[],missing=[];
  for(const w of wanted){const key=canonicalShoppingKeyV105(w);let hits=active.filter(r=>canonicalShoppingKeyV105(r.title)===key);
    if(hits.length===1){resolved.push(hits[0]);continue;}if(hits.length>1){ambiguous.push({query:w,names:hits.map(x=>x.title)});continue;}
    const ranked=active.map(r=>({r,s:scoreShopTargetV107(w,r)})).filter(x=>x.s>0).sort((x,y)=>y.s-x.s);if(!ranked.length){missing.push(w);continue;}const top=ranked[0].s;hits=ranked.filter(x=>x.s===top).map(x=>x.r);
    if(hits.length!==1||top<20)ambiguous.push({query:w,names:hits.slice(0,6).map(x=>x.title)});else resolved.push(hits[0]);
  }
  return{resolved:[...new Map(resolved.map(x=>[Number(x.id),x])).values()],ambiguous,missing};
}
markShoppingByText=async function(env,chatId,text,status){
  const list=await getDefaultShoppingList(env,chatId,false);if(!list)return{changed:0,names:[],ambiguous:[],missing:[]};const rows=await getShoppingItems(env,chatId,list.id);const r=resolveShoppingTextTargetsV113(rows,text);if(r.ambiguous.length)return{changed:0,names:[],list,...r};
  let changed=0;const names=[];for(const x of r.resolved){if(x.status!==status){await env.DB.prepare(`UPDATE smart_list_items SET status=?,updated_at=? WHERE id=? AND chat_id=?`).bind(status,new Date().toISOString(),Number(x.id),chatId).run();await writeAudit(env,chatId,{action:'update',entityType:'shopping_item',entityId:String(x.id),summary:`تحديث ${x.title}`,before:{status:x.status},after:{status},undo:{type:'restore_list_item_status',id:Number(x.id),status:x.status}});changed++;names.push(x.title);}}
  if(changed)v113ArmMutationReceipt(chatId,'shopping-mark');return{changed,names,list,ambiguous:[],missing:r.missing};
};
removeShoppingByText=async function(env,chatId,text){
  const list=await getDefaultShoppingList(env,chatId,false);if(!list)return{changed:0,names:[],ambiguous:[],missing:[]};const rows=await getShoppingItems(env,chatId,list.id);const r=resolveShoppingTextTargetsV113(rows,text);if(r.ambiguous.length)return{changed:0,names:[],list,...r};
  let changed=0;const names=[];for(const x of r.resolved){await env.DB.prepare(`DELETE FROM smart_list_items WHERE id=? AND chat_id=?`).bind(Number(x.id),chatId).run();await writeAudit(env,chatId,{action:'delete',entityType:'shopping_item',entityId:String(x.id),summary:`حذف من المشتريات: ${x.title}`,before:x,undo:{type:'restore_deleted_list_item',row:x},strict:true});changed++;names.push(x.title);}
  if(changed)v113ArmMutationReceipt(chatId,'shopping-remove');return{changed,names,list,ambiguous:[],missing:r.missing};
};

async function forgetWorldGraphForMemoryV113(env,chatId,memory){
  const text=String(memory||'').trim();if(!text)return 0;let n=0;
  const res=await env.DB.prepare(`DELETE FROM life_edges WHERE chat_id=? AND relation='memory_fact' AND object_value=?`).bind(chatId,text).run();n+=Number(res?.meta?.changes||0);
  const patterns=[{re:/^(.+?)\s+(?:مراتي|زوجتي)$/iu,rel:'spouse'},{re:/^(.+?)\s+(?:ابني|ابنى|بنتي)$/iu,rel:'child'},{re:/^(.+?)\s+(?:اخويا|أخويا|اختي|أختي)$/iu,rel:'sibling'}];
  for(const p of patterns){const m=text.match(p.re);if(!m)continue;const self=await env.DB.prepare(`SELECT id FROM life_entities WHERE chat_id=? AND entity_type='person' AND normalized_name=? LIMIT 1`).bind(chatId,normalizeArabicLoose('أنا')).first();const other=await env.DB.prepare(`SELECT id FROM life_entities WHERE chat_id=? AND entity_type='person' AND normalized_name=? LIMIT 1`).bind(chatId,normalizeArabicLoose(m[1].trim())).first();if(self?.id&&other?.id){const d=await env.DB.prepare(`DELETE FROM life_edges WHERE chat_id=? AND from_entity_id=? AND relation=? AND to_entity_id=?`).bind(chatId,Number(self.id),p.rel,Number(other.id)).run();n+=Number(d?.meta?.changes||0);}break;}
  return n;
}
forgetUserMemory=async function(env,chatId,query){
  const q=normalizeArabicLoose(query);if(!q)return 0;const rows=(await env.DB.prepare(`SELECT id,memory,normalized FROM user_memories WHERE chat_id=? ORDER BY id DESC LIMIT 200`).bind(chatId).all())?.results||[];
  const hits=rows.filter(r=>String(r.normalized||'').includes(q)||q.includes(String(r.normalized||'')));if(!hits.length)return 0;
  await env.DB.batch(hits.map(r=>env.DB.prepare(`DELETE FROM user_memories WHERE id=? AND chat_id=?`).bind(Number(r.id),chatId)));
  for(const r of hits)await forgetWorldGraphForMemoryV113(env,chatId,r.memory);
  await env.DB.prepare(`DELETE FROM life_entities WHERE chat_id=? AND id NOT IN (SELECT from_entity_id FROM life_edges WHERE chat_id=? UNION SELECT to_entity_id FROM life_edges WHERE chat_id=? AND to_entity_id IS NOT NULL)`).bind(chatId,chatId,chatId).run();
  v113ArmMutationReceipt(chatId,'memory-forget');return hits.length;
};

// Full user-data wipe (transport ledgers for the currently executing Telegram update are intentionally retained for reliability).
clearEverythingV105=async function(env,chatId){
  const count=async table=>Number((await env.DB.prepare(`SELECT COUNT(*) AS c FROM ${table} WHERE chat_id=?`).bind(chatId).first())?.c||0);
  const before={reminders:await count('reminders'),rules:await count('schedule_rules')+await count('recurring_rules'),prayer:await count('prayer_rules'),memories:await count('user_memories'),projects:await count('projects'),waiting:await count('waiting_items'),watches:await count('live_watches'),shopping:await count('smart_list_items'),entities:await count('life_entities'),edges:await count('life_edges')};
  const tables=['event_dependencies','reminder_fires','schedule_fires','prayer_rule_fires','reminders','schedule_rules','recurring_rules','prayer_rules','conversation_messages','pending_dialogs','pending_conflicts','pending_requests','operation_receipts','user_memories','live_watches','life_inbox','project_tasks','projects','waiting_items','daily_brief_fires','agent_settings','user_rate_limits','action_audit','runtime_failures','shopping_sessions','smart_list_items','smart_lists','life_edges','life_entities','user_profiles'];
  await env.DB.batch(tables.map(t=>env.DB.prepare(`DELETE FROM ${t} WHERE chat_id=?`).bind(chatId)));
  v113ArmMutationReceipt(chatId,'clear-everything');return before;
};

// Preserve shopping metadata in every undo/rollback path.
const __V112_restoreShoppingListSnapshotV1034=restoreShoppingListSnapshotV1034;
restoreShoppingListSnapshotV1034=async function(env,chatId,snapshot){
  const list=snapshot?.list;if(!list?.id)return false;const now=new Date().toISOString();
  await env.DB.prepare(`INSERT OR REPLACE INTO smart_lists(id,chat_id,name,normalized_name,list_type,active,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)`).bind(Number(list.id),chatId,String(list.name||'مشتريات'),String(list.normalized_name||normalizeArabicLoose(list.name||'مشتريات')),String(list.list_type||'shopping'),Number(list.active??1),String(list.created_at||now),now).run();
  for(const x of snapshot.items||[])await env.DB.prepare(`INSERT OR REPLACE INTO smart_list_items(id,list_id,chat_id,title,normalized_title,quantity,status,position,created_at,updated_at,meta_json) VALUES (?,?,?,?,?,?,?,?,?,?,?)`).bind(Number(x.id),Number(list.id),chatId,String(x.title||''),String(x.normalized_title||canonicalShoppingKeyV105(x.title||'')),x.quantity??null,String(x.status||'pending'),Number(x.position||0),String(x.created_at||now),now,String(x.meta_json||'{}')).run();
  return true;
};
snapshotV102ShoppingMutation=async function(env,chatId,items){
  const existingList=await getDefaultShoppingList(env,chatId,false),before=[];
  if(existingList)for(const title of items){const n=canonicalShoppingKeyV105(String(title||''));if(!n)continue;const row=await env.DB.prepare(`SELECT * FROM smart_list_items WHERE list_id=? AND chat_id=? AND normalized_title=? ORDER BY id DESC LIMIT 1`).bind(Number(existingList.id),chatId,n).first();if(row)before.push({...row});}
  const audit=await env.DB.prepare(`SELECT COALESCE(MAX(id),0) AS id FROM action_audit WHERE chat_id=?`).bind(chatId).first();return{listExisted:!!existingList,listId:Number(existingList?.id||0),before,auditId:Number(audit?.id||0)};
};
rollbackV102ShoppingMutation=async function(env,chatId,snapshot,result){
  const beforeById=new Map((snapshot?.before||[]).map(x=>[Number(x.id),x])),statements=[];
  for(const id0 of result?.ids||[]){const id=Number(id0),prev=beforeById.get(id);if(prev)statements.push(env.DB.prepare(`UPDATE smart_list_items SET status=?,title=?,normalized_title=?,quantity=?,position=?,meta_json=?,updated_at=? WHERE id=? AND chat_id=?`).bind(prev.status,prev.title,prev.normalized_title,prev.quantity,Number(prev.position||0),String(prev.meta_json||'{}'),new Date().toISOString(),id,chatId));else statements.push(env.DB.prepare(`DELETE FROM smart_list_items WHERE id=? AND chat_id=?`).bind(id,chatId));}
  if(statements.length)await env.DB.batch(statements);await env.DB.prepare(`DELETE FROM action_audit WHERE chat_id=? AND id>? AND entity_type='shopping_items'`).bind(chatId,Number(snapshot?.auditId||0)).run();
  if(!snapshot?.listExisted&&Number(result?.list?.id||0)){const c=await env.DB.prepare(`SELECT COUNT(*) AS c FROM smart_list_items WHERE list_id=? AND chat_id=?`).bind(Number(result.list.id),chatId).first();if(Number(c?.c||0)===0)await env.DB.prepare(`DELETE FROM smart_lists WHERE id=? AND chat_id=?`).bind(Number(result.list.id),chatId).run();}
};

// Transport-level exactly-once response receipt keyed by Telegram update_id.
function v113SetUpdateContext(chatId,updateId){if(!chatId)return;V113_ACTIVE_UPDATE_CONTEXT.set(String(chatId),{updateId:updateId==null?'':String(updateId),scope:null});}
function v113ClearUpdateContext(chatId){V113_ACTIVE_UPDATE_CONTEXT.delete(String(chatId));}
function v113ArmMutationReceipt(chatId,scope){const c=V113_ACTIVE_UPDATE_CONTEXT.get(String(chatId));if(c&&c.updateId)c.scope=String(scope||'mutation').slice(0,120);}
function v113DisarmMutationReceipt(chatId){const c=V113_ACTIVE_UPDATE_CONTEXT.get(String(chatId));if(c)c.scope=null;}
const V113_RECEIPT_ENVELOPE='__V113_RESPONSE_SEQUENCE__:';
async function v113PersistCurrentResponse(env,chatId,text,replyMarkup=null){
  const c=V113_ACTIVE_UPDATE_CONTEXT.get(String(chatId));if(!c?.updateId||!c.scope||!env?.DB)return;
  const fp=`tg:${c.updateId}:mutation`,now=new Date().toISOString();
  const row=await env.DB.prepare(`SELECT id,response_text FROM operation_receipts WHERE chat_id=? AND fingerprint=? AND state='committed' ORDER BY id DESC LIMIT 1`).bind(String(chatId),fp).first();
  let entries=[];if(String(row?.response_text||'').startsWith(V113_RECEIPT_ENVELOPE)){try{entries=JSON.parse(String(row.response_text).slice(V113_RECEIPT_ENVELOPE.length));if(!Array.isArray(entries))entries=[];}catch{entries=[];}}
  else if(row?.response_text)entries=[{text:String(row.response_text),reply_markup:null,scope:'legacy'}];
  entries.push({text:String(text||'').slice(0,12000),reply_markup:replyMarkup||null,scope:String(c.scope||'mutation')});entries=entries.slice(-8);
  const encoded=V113_RECEIPT_ENVELOPE+JSON.stringify(entries);
  if(row?.id)await env.DB.prepare(`UPDATE operation_receipts SET response_text=?,updated_at=? WHERE id=? AND chat_id=?`).bind(encoded,now,Number(row.id),String(chatId)).run();
  else await env.DB.prepare(`INSERT INTO operation_receipts(chat_id,fingerprint,action,state,response_text,created_at,updated_at) VALUES (?,?,?,?,?,?,?)`).bind(String(chatId),fp,'telegram_mutation','committed',encoded,now,now).run();
}
async function v113ReplayCommittedUpdateIfAny(env,chatId,updateId){
  if(!updateId||!env?.DB)return false;const fp=`tg:${String(updateId)}:mutation`;const row=await env.DB.prepare(`SELECT response_text FROM operation_receipts WHERE chat_id=? AND fingerprint=? AND state='committed' ORDER BY id DESC LIMIT 1`).bind(String(chatId),fp).first();if(!row?.response_text)return false;
  const raw=String(row.response_text);let entries=[];if(raw.startsWith(V113_RECEIPT_ENVELOPE)){try{entries=JSON.parse(raw.slice(V113_RECEIPT_ENVELOPE.length));}catch{entries=[];}}else entries=[{text:raw,reply_markup:null}];
  if(!Array.isArray(entries)||!entries.length)return false;for(const e of entries){if(!String(e?.text||'').trim())continue;await sendText(env,chatId,String(e.text),e.reply_markup||null);}return true;
}
const __V112_sendText=sendText;
sendText=async function(env,chatId,text,replyMarkup=null){await v113PersistCurrentResponse(env,chatId,text,replyMarkup);return __V112_sendText(env,chatId,text,replyMarkup);};
const __V112_editOrSend=editOrSend;
editOrSend=async function(env,chatId,messageId,text,replyMarkup=null){await v113PersistCurrentResponse(env,chatId,text,replyMarkup);return __V112_editOrSend(env,chatId,messageId,text,replyMarkup);};

// Arm intent mutations before their first user-visible result. A conflict/confirmation response is also safely replayable.
const __V112_executeIntent=executeIntent;
const __V112_persistWorldUpdatesSafely=persistWorldUpdatesSafely;
persistWorldUpdatesSafely=async function(env,chatId,intent){intent._v113_world_updates_deferred=true;};
executeIntent=async function(env,chatId,intent,options={}){
  const action=String(intent?.action||'');const mutating=new Set(['create','delete','update','manage_rule','bulk_shift','bulk_delete','shopping']);if(mutating.has(action))v113ArmMutationReceipt(chatId,`intent-${action}`);
  let ok=false;try{const out=await __V112_executeIntent(env,chatId,intent,options);ok=true;
    const committed=!!intent?._v113_operation_committed;
    if(committed&&Array.isArray(intent?.world_updates)&&intent.world_updates.length)await __V112_persistWorldUpdatesSafely(env,chatId,intent);
    return out;
  }finally{if(!ok)v113DisarmMutationReceipt(chatId);}
};

// Safe schedule deletes: cleanup dependencies; optional silent mode is used by callbacks.
cancelReminder=async function(env,chatId,id,intent=null){
  const silent=!!intent?._v113_silent,before=await env.DB.prepare(`SELECT * FROM reminders WHERE id=? AND chat_id=? LIMIT 1`).bind(id,chatId).first();
  const res=await env.DB.prepare(`UPDATE reminders SET cancelled=1 WHERE id=? AND chat_id=? AND cancelled=0 AND sent=0`).bind(id,chatId).run();const changed=Number(res?.meta?.changes||0);
  if(changed&&before){await env.DB.prepare(`DELETE FROM event_dependencies WHERE chat_id=? AND ((source_type='reminder' AND source_id=?) OR (target_type='reminder' AND target_id=?))`).bind(chatId,Number(id),Number(id)).run();await writeAudit(env,chatId,{action:'delete',entityType:'reminder',entityId:String(id),summary:`إلغاء: ${before.title}`,before,undo:{type:'restore_cancelled_reminder',id:Number(id)},strict:true});if(intent)intent._v113_operation_committed=true;v113ArmMutationReceipt(chatId,'delete-reminder');}
  const final=(changed?'🗑️ تم إلغاء الموعد.':'مش لاقي الموعد المطلوب أو الموعد خلص بالفعل.')+modelFooter(env,chatId,intent);if(!silent){await sendText(env,chatId,final,quickMenuKeyboard());await saveConversationMessage(env,chatId,'assistant',final);}return{changed,final};
};
deleteScheduleRule=async function(env,chatId,id,intent=null){
  const silent=!!intent?._v113_silent,before=await env.DB.prepare(`SELECT * FROM schedule_rules WHERE id=? AND chat_id=? LIMIT 1`).bind(id,chatId).first();const res=await env.DB.batch([env.DB.prepare(`DELETE FROM schedule_fires WHERE rule_id=? AND chat_id=?`).bind(id,chatId),env.DB.prepare(`DELETE FROM schedule_rules WHERE id=? AND chat_id=?`).bind(id,chatId)]);const deleted=Number(res?.[1]?.meta?.changes||0);
  if(deleted&&before){await writeAudit(env,chatId,{action:'delete',entityType:'schedule_rule',entityId:String(id),summary:`حذف تكرار: ${before.title}`,before,undo:{type:'restore_schedule_rule',row:before},strict:true});if(intent)intent._v113_operation_committed=true;v113ArmMutationReceipt(chatId,'delete-rule');}
  const final=(deleted?'🗑️ تم حذف التكرار بالكامل.':'مش لاقي التكرار المطلوب.')+modelFooter(env,chatId,intent);if(!silent){await sendText(env,chatId,final,quickMenuKeyboard());await saveConversationMessage(env,chatId,'assistant',final);}return{deleted,final};
};

async function clearAllUserScheduleV113(env,chatId){
  const res=await env.DB.batch([
    env.DB.prepare(`DELETE FROM event_dependencies WHERE chat_id=?`).bind(chatId),env.DB.prepare(`DELETE FROM reminder_fires WHERE chat_id=?`).bind(chatId),env.DB.prepare(`DELETE FROM schedule_fires WHERE chat_id=?`).bind(chatId),env.DB.prepare(`DELETE FROM prayer_rule_fires WHERE chat_id=?`).bind(chatId),env.DB.prepare(`DELETE FROM reminders WHERE chat_id=?`).bind(chatId),env.DB.prepare(`DELETE FROM schedule_rules WHERE chat_id=?`).bind(chatId),env.DB.prepare(`DELETE FROM recurring_rules WHERE chat_id=?`).bind(chatId),env.DB.prepare(`DELETE FROM prayer_rules WHERE chat_id=?`).bind(chatId),env.DB.prepare(`DELETE FROM pending_dialogs WHERE chat_id=?`).bind(chatId),env.DB.prepare(`DELETE FROM pending_conflicts WHERE chat_id=?`).bind(chatId)
  ]);v113ArmMutationReceipt(chatId,'clear-schedule');return Number(res?.[4]?.meta?.changes||0)+Number(res?.[5]?.meta?.changes||0)+Number(res?.[6]?.meta?.changes||0)+Number(res?.[7]?.meta?.changes||0);
}

// Arm high-confidence direct mutations, but disarm if the direct handler declines the message.
const __V112_handleV10DirectCommands=handleV10DirectCommands;
handleV10DirectCommands=async function(env,chatId,text,options={}){
  const raw=String(text||'').trim(),should=looksLikeMutationTextV113(raw);if(should)v113ArmMutationReceipt(chatId,'direct-v10');
  const session=await getActiveShoppingSession(env,chatId);
  if(session){
    const cases=[
      {re:/^(?:جبت|اشتريت|خدت)\s+(.+)$/iu,status:'bought',ok:'✅ تمام، علمت: ',none:'ملقتش الأصناف دي في القائمة.'},
      {re:/^(?:ملقتش|مش\s+موجود|مفيش)\s+(.+)$/iu,status:'unavailable',ok:'🚫 سجلت إنها مش موجودة: ',none:'ملقتش الأصناف دي في القائمة.'},
      {re:/^(?:سيب|سيبلي|مش\s+هاجيب)\s+(.+)$/iu,status:'skipped',ok:'⏭️ تمام، سيبت: ',none:'ملقتش الأصناف دي في القائمة.'}
    ];
    for(const c of cases){const m=raw.match(c.re);if(!m)continue;const r=await markShoppingByText(env,chatId,m[1],c.status);if(r.ambiguous?.length){await sendText(env,chatId,`لقيت أكتر من صنف ممكن يكون المقصود بـ «${r.ambiguous[0].query}»: ${r.ambiguous[0].names.join('، ')}. اكتب الاسم بشكل أوضح.`);return true;}await sendText(env,chatId,r.changed?c.ok+r.names.join('، '):c.none);if(r.changed)await showShoppingList(env,chatId,null,{startSession:false});return true;}
  }
  const handled=await __V112_handleV10DirectCommands(env,chatId,raw,options);
  if(should&&!handled)v113DisarmMutationReceipt(chatId);
  if(handled&&looksLikeShoppingMutationV113(raw))try{await showShoppingList(env,chatId,null,{startSession:false});}catch{}
  return handled;
};
const __V112_handleLifeDirectCommands=handleLifeDirectCommands;
handleLifeDirectCommands=async function(env,chatId,text){const t=normalizeArabicLoose(String(text||''));const should=/(?:افتكر|انسى|انسي|حط|سجل|مستني|مستنى|تابعلي|راقبلي|وقف متابعه|وقف متابعة)/u.test(t);if(should)v113ArmMutationReceipt(chatId,'direct-life');const handled=await __V112_handleLifeDirectCommands(env,chatId,text);if(should&&!handled)v113DisarmMutationReceipt(chatId);return handled;};
const __V112_handleDirectCommands=handleDirectCommands;
handleDirectCommands=async function(env,chatId,text){const t=normalizeArabicLoose(String(text||''));const should=/^(?:\/delete|\/undo|تراجع|رجع|ارجع|احذف)/u.test(t);if(should)v113ArmMutationReceipt(chatId,'direct-command');const handled=await __V112_handleDirectCommands(env,chatId,text);if(should&&!handled)v113DisarmMutationReceipt(chatId);return handled;};

// Callback hardening + safe deletion path. Other mutating callbacks are receipt-armed before delegating.
const __V112_handleCallbackQuery=handleCallbackQuery;
handleCallbackQuery=async function(query,env){
  const data=String(query?.data||''),chatId=String(query?.message?.chat?.id??query?.from?.id??''),messageId=Number(query?.message?.message_id||0);if(!chatId)return;
  if(!isAllowedUserV113(env,chatId)){await sendText(env,chatId,'⛔ البوت غير متاح للحساب ده.');return;}
  if(data.startsWith('do:delete:one:')){const id=Number(data.split(':')[3]||0);if(id){await cancelReminder(env,chatId,id,{_v113_silent:true});return sendAllSchedule(env,chatId,messageId);}return;}
  if(data.startsWith('do:delete:rule:')){const id=Number(data.split(':')[3]||0);if(id){await deleteScheduleRule(env,chatId,id,{_v113_silent:true});return showRecurringRules(env,chatId,messageId);}return;}
  if(data==='do:clear_all_user_schedule'){const n=await clearAllUserScheduleV113(env,chatId);return editOrSend(env,chatId,messageId,`✅ تم مسح كل مواعيدك وكل التكرارات بما فيها التذكيرات المرتبطة بالصلاة.\n🗑️ المحذوف: ${n}`,mainMenuKeyboard(env,chatId));}
  if(data==='do:clear_shopping'){v113ArmMutationReceipt(chatId,'clear-shopping');const r=await deleteShoppingListV1034(env,chatId);return editOrSend(env,chatId,messageId,r.deleted?`✅ تم مسح قائمة المشتريات (${r.count} عنصر). تقدر ترجعها بـ /undo.`:'✅ قائمة المشتريات فارغة أصلًا.',{inline_keyboard:[[{text:'↩️ إدارة المسح',callback_data:'panel:danger'}]]});}
  if(/^(?:shop:toggle:|shop:finish:|audit:undo$|setting:toggle:|rule:toggle:|rule:skip:|rule:pause1d:|rule:resume:|prayer:toggle:|prayer:skip:|prayer:delete:|rem:s10:|rem:s60:|do:clear_|bulk:confirm$|conflict:force$)/.test(data))v113ArmMutationReceipt(chatId,`callback-${data.slice(0,90)}`);
  return __V112_handleCallbackQuery(query,env);
};

const __V112_toggleShoppingItemCallback=toggleShoppingItemCallback;
toggleShoppingItemCallback=async function(env,chatId,messageId,itemId){v113ArmMutationReceipt(chatId,'shopping-toggle');return __V112_toggleShoppingItemCallback(env,chatId,messageId,itemId);};
const __V112_finishShoppingSessionCallback=finishShoppingSessionCallback;
finishShoppingSessionCallback=async function(env,chatId,messageId,listId){v113ArmMutationReceipt(chatId,'shopping-finish');return __V112_finishShoppingSessionCallback(env,chatId,messageId,listId);};

// Fix old direct shopping callers now that removeShoppingByText returns a structured result.
const __V112_tryDirectShoppingDeleteV1034=tryDirectShoppingDeleteV1034;
tryDirectShoppingDeleteV1034=async function(env,chatId,raw){
  const t=normalizeArabicLoose(String(raw||'').trim());
  if(/^(?:احذف|امسح|شيل|فضي|افرغ)\s+(?:(?:كل|قائمه|قايمه|قائمة|قايمة)\s+)?(?:المشتريات|قائمه\s+المشتريات|قايمه\s+المشتريات|قائمة\s+المشتريات|قايمة\s+المشتريات)(?:\s+كلها)?$/u.test(t))return __V112_tryDirectShoppingDeleteV1034(env,chatId,raw);
  let m=String(raw||'').trim().match(/^(?:امسح|شيل|احذف)\s+(.+?)\s+(?:من\s+)?(?:المشتريات|قائمة\s+المشتريات|قائمه\s+المشتريات|قايمة\s+المشتريات|قايمه\s+المشتريات|القائمة|القايمة)$/iu);if(!m){const session=await getActiveShoppingSession(env,chatId);if(session)m=String(raw||'').trim().match(/^(?:امسح|شيل|احذف)\s+(.+)$/iu);}if(!m)return false;
  const r=await removeShoppingByText(env,chatId,m[1]);if(r.ambiguous?.length){await sendText(env,chatId,`لقيت أكتر من صنف ممكن يكون المقصود بـ «${r.ambiguous[0].query}»: ${r.ambiguous[0].names.join('، ')}. اكتب الاسم بشكل أوضح.`);return true;}await sendText(env,chatId,r.changed?`🗑️ شلت ${r.changed} من قائمة المشتريات. تقدر ترجع آخر حذف بـ /undo.`:'ملقتش الصنف ده في المشتريات.');if(r.changed)await showShoppingList(env,chatId,null,{startSession:false});return true;
};

// Telegram networking: bounded timeout + retry on 429/network/5xx.
telegramApi=async function(env,method,payload){
  payload=localizeTelegramPayloadV1043(method,payload);try{const r=await fetchWithTimeoutV113(`https://api.telegram.org/bot${env.TELEGRAM_BOT_TOKEN}/${method}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload||{})},V113_TELEGRAM_TIMEOUT_MS);const raw=await r.text();try{return JSON.parse(raw);}catch{return{ok:false,error_code:r.status,description:`أعاد تيليجرام استجابة غير صالحة (${r.status})`};}}
  catch(e){return{ok:false,error_code:599,description:`Telegram network/timeout: ${safeError(e)}`};}
};
telegramApiWithRetry=async function(env,method,payload,maxRetries=1){
  let last=null;for(let attempt=0;attempt<=maxRetries;attempt++){last=await telegramApi(env,method,payload);if(last?.ok)return last;const code=Number(last?.error_code||0),retry=Number(last?.parameters?.retry_after||0);if(attempt>=maxRetries)return last;if(code===429&&retry>0){await sleep(Math.min(5000,retry*1000+150));continue;}if(code===599||code>=500){await sleep(150+attempt*250);continue;}return last;}return last||{ok:false,description:'فشل طلب تيليجرام'};
};

// Cron recovers multiple chats per tick instead of one.
drainPendingTelegramInboxV106=async function(env){
  if(!env?.DB)return;const rows=(await env.DB.prepare(`SELECT DISTINCT chat_id FROM telegram_inbox_v106 WHERE status='pending' OR (status='processing' AND (lease_until IS NULL OR lease_until<=?)) LIMIT ?`).bind(new Date().toISOString(),V113_RECOVERY_CHAT_BATCH).all())?.results||[];
  for(const row of rows)await drainTelegramInboxV106(env,String(row.chat_id));
};

async function reliabilityReadyV113(request,env){
  const url=new URL(request.url),key=adminKeyFromRequestV115(request);if(!env.SETUP_KEY||key!==env.SETUP_KEY)return json({ok:false,error:'غير مصرح'},401);
  ...
  try{
    ...
    for(const model of MODEL_CHAIN){
      const c=new AbortController(),timeout=Math.min(5500,Math.max(1800,Number(model.timeoutMs||2500)+350)),timer=setTimeout(()=>c.abort(),timeout),started=Date.now();
      try{
...
// Versioned identity.
handleV11Identity=async function(text,env,chatId){if(!isV11IdentityQuestion(text))return false;await sendText(env,chatId,'أنا سوبر إيجنت V11.6.1 🤖 — مساعدك الشخصي لتنظيم يومك ومواعيدك ومشترياتك وذاكرتك ومهامك من الكلام الطبيعي.');return true;};

function runV113PureSelfTests(){
  const t=[];const add=(name,ok,detail='')=>t.push({name,ok:!!ok,detail});
  add('v113 exactly 3 models',MODEL_CHAIN.length===3,String(MODEL_CHAIN.length));
  add('v113 no fast chat for FX',v112LooksLikeDirectChat('سعر الدولار كام دلوقتي؟')===false);
  add('v113 no fast chat for weather',v112LooksLikeDirectChat('الجو عامل ايه النهارده؟')===false);
  add('v113 no fast chat for personal memory',v112LooksLikeDirectChat('اسم مراتي ايه؟')===false);
  add('v113 casual still fast',v112LooksLikeDirectChat('عامل اي')===true);
  add('v113 current detector',needsCurrentExternalDataV113('سعر الذهب كام النهارده')===true);
  add('v113 current office holder detector',needsCurrentExternalDataV113('مين رئيس مصر دلوقتي؟')===true&&v112LooksLikeDirectChat('مين رئيس مصر دلوقتي؟')===false);
  add('v113 natural shopping is mutation',looksLikeMutationTextV113('بص عاوز اشتري لبن وبيض')===true);
  add('v113 holiday context is lazy-on-demand',needsHolidayContextV113('فكرني في اجازة رسمية')===true);
  add('v113 allowlist parser',allowedChatIdsV113({ADMIN_CHAT_ID:'1',ALLOWED_CHAT_IDS:'2, 3'}).size===3);
  const rows=[{id:1,title:'لبن كامل الدسم',status:'pending'},{id:2,title:'لبن خالي الدسم',status:'pending'}];const rr=resolveShoppingTextTargetsV113(rows,'لبن');
  add('v113 fuzzy shopping ambiguity blocks mutation',rr.ambiguous.length===1,JSON.stringify(rr));
  const longList='بص عاوز اشتري\nعيش تورتيلا\nعيش توست\nفينو اسود\nفصوص رومي\nشيدر طبيعي\nكاجو\nفستق\nكوفي شيك\nحليب دينا\nايس كريم دينا\nوبطاطس طبيعية';
  add('v113 explicit list keeps 11',extractExplicitShoppingUnitsV11(longList).length===11,String(extractExplicitShoppingUnitsV11(longList).length));
  const p={operations:extractExplicitShoppingUnitsV11(longList).map(title=>({op:'add',title,meta:{}}))};const aft={items:[{id:1,title:'بطاطس طبيعية',status:'pending',meta_json:'{}'}]};
  add('v113 shopping verifier rejects 1 of 11',verifyShoppingPlanV113({items:[]},aft,p).ok===false,JSON.stringify(verifyShoppingPlanV113({items:[]},aft,p)));
  add('v113 basic reality no prayer fetch needed',basicRealityContextV113({timezone:'Africa/Cairo',city:'Cairo',country:'Egypt',country_code:'EG'}).prayers.today===null);
  add('v113 live general safety',needsLiveNews('سعر الدولار كام')===false&&needsCurrentExternalDataV113('سعر الدولار كام')===true);
  const passed=t.filter(x=>x.ok).length;return{ok:passed===t.length,passed,total:t.length,tests:t};
}
...
const V115_SCHEMA_VERSION='11.5.1';
...
v112CallPlainChat=async function(env,model,text,history=[],timeoutCapMs=V112_CHAT_TIMEOUT_MS){
  const controller=new AbortController();const cap=Math.max(250,Number(timeoutCapMs||V112_CHAT_TIMEOUT_MS));const timeout=Math.min(Number(model?.timeoutMs||V112_CHAT_TIMEOUT_MS),V112_CHAT_TIMEOUT_MS,cap);const timer=setTimeout(()=>controller.abort(),timeout);
  try{
    const recent=(Array.isArray(history)?history:[]).slice(-8).map(m=>({role:m.role==='assistant'?'assistant':'user',content:String(m.content||'').slice(0,1200)}));
    ...
    const req=new Request(OMNIAI_INTERNAL_URL,{method:'POST',headers:{Authorization:'Bearer '+env.OMNIAI_API_KEY,'Content-Type':'application/json'},body:JSON.stringify({model:model.id,messages,temperature:0.35,max_tokens:900,stream:false}),signal:controller.signal});
    const res=await env.OMNIAI_SERVICE.fetch(req);if(!res.ok){const e=Object.assign(new Error('chat_http_'+res.status),{httpStatus:res.status});throw e;}const data=await res.json();const out=v11ExtractModelText(data).trim();if(!out)throw new Error('chat_empty');return out;
...
tryV112FastChat=async function(env,chatId,text,history=[]){
  ...
  if(!v112LooksLikeDirectChat(text))return false;
  await enforceAiRateLimit(env,chatId);
  const started=Date.now(),deadline=started+V112_CHAT_TOTAL_BUDGET_MS,errors=[];
  for(const model of V112_CHAT_MODELS){const remaining=deadline-Date.now();if(remaining<250)break;const a=Date.now();try{const answer=await v112CallPlainChat(env,model,text,history,remaining);try{await recordModelResult(env,model,true,Date.now()-a,null);}catch{}await sendText(env,chatId,answer,quickMenuKeyboard());await saveConversationMessage(env,chatId,'assistant',answer);return true;}catch(e){errors.push(e);try{await recordModelResult(env,model,false,Date.now()-a,String(e?.message||e));}catch{}}}
  ...
  const allRateLimited=errors.length>0&&errors.every(e=>Number(Object(e).httpStatus||0)===429);
  ...
  const e=Object.assign(new Error(`V11.6 transient chat failure ${incident}`),{retryable:true});throw e;
};
...
async function purgeTransportForChatV115(env,chatId,throughUpdateId){
  const id=String(throughUpdateId||''),numeric=/^\d+$/.test(id),stmts=[env.DB.prepare(`DELETE FROM operation_receipts WHERE chat_id=?`).bind(String(chatId)),env.DB.prepare(`DELETE FROM conversation_messages WHERE chat_id=?`).bind(String(chatId))];
...
drainTelegramInboxV106=async function(env,chatId,origin=''){
  ...
  for(let retry=0;retry<V106_LEASE_RETRY_COUNT;retry++){if(await acquireChatLeaseV106(env,chatId,owner)){acquired=true;break;}await sleepV106(V106_LEASE_RETRY_DELAY_MS+Math.floor(Math.random()*50));}if(!acquired){if(origin&&await hasRunnableInboxV106(env,chatId)){const delay=Math.min(1800,Math.max(700,Math.floor(V106_INBOX_LEASE_MS/20)))+Math.floor(Math.random()*120);await sleepV106(delay);await triggerDrainContinuationV106(env,chatId,origin);}return;}
  try{
    for(let i=0;i<V106_INBOX_BATCH_SIZE;i++){
      ...
      let hb=null;try{
        await renewChatLeaseV106(env,chatId,owner);await renewInboxLeaseV115(env,chatId,row.update_id);hb=setInterval(()=>{Promise.all([renewChatLeaseV106(env,chatId,owner),renewInboxLeaseV115(env,chatId,row.update_id)]).catch(e=>console.warn('V11.5.1 lease heartbeat failed',safeError(e)));},V115_QUEUE_HEARTBEAT_MS);
