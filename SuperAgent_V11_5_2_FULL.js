/* SuperAgent V11.5.2 FINAL — fixed Google chain; V11.5.1 hardening retained; durable inbox lease-collision self-recovery, bounded contention wait, and strict direct-chat deadline; certified staging matrix. */
/* SuperAgent V11.5.2 FINAL — Google-only Lite-priority chain; stronger live-proven third fallback; V11.5 hardening retained; no AI router. */
/* SuperAgent V11.5.2 FULL — hardening over V11.4; Google Lite-priority 3-model chain; no AI router. */
...
async function selfTestEndpoint(request,env){const url=new URL(request.url);const key=adminKeyFromRequestV115(request);if(!env.SETUP_KEY||key!==env.SETUP_KEY)return json({ok:false,error:"غير مصرح"},401);return json({version:V10_VERSION,...runV10SelfTests()});}
...
function needsCurrentExternalDataV113(text){
...
// Versioned identity.
handleV11Identity=async function(text,env,chatId){if(!isV11IdentityQuestion(text))return false;await sendText(env,chatId,'أنا سوبر إيجنت V11.5.2 🤖 — مساعدك الشخصي لتنظيم يومك ومواعيدك ومشترياتك وذاكرتك ومهامك من الكلام الطبيعي.');return true;};
...
tryV112FastChat=async function(env,chatId,text,history=[]){
  ...
  const e=new Error(`V11.5.1 transient chat failure ${incident}`);e.retryable=true;throw e;
};
