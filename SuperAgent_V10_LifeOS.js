...
[{text:"🛒 المشتريات",callback_data:"panel:shopping"},{text:"📥 Inbox",callback_data:"panel:inbox"}],
[{text:"🧠 عالمى وذاكرتي",callback_data:"panel:world"},{text:"📜 سجل التغييرات",callback_data:"panel:audit"}],
...
const lines=[
"🎛️ Super Agent V10 — Life OS",
"",
`📅 القادم: ${Number(one?.c||0)} · 🔁 التكرارات: ${Number(rules?.c||0)+Number(prayerRules?.c||0)}`,
`🛒 مشتريات متبقية: ${pendingShop} · 📥 Inbox: ${Number(inbox?.c||0)}`,
`🧠 كيانات في World Model: ${Number(world?.c||0)}`,
"",
...
if(["/undo","تراجع","رجع اخر حاجه","رجع اخر حاجة"].includes(t)){
...
await persistWorldUpdatesFromIntent(env,chatId,intent);
...
return`
أنت Super Agent V10 Life OS: سكرتير شخصي يفهم حياة المستخدم كحالة مستمرة، وليس مجرد بوت تذكيرات. المستخدم يتكلم غالبًا بالمصري.
...
async function writeAudit(env,chatId,{action,entityType,entityId=null,summary,before={},after={},undo={}}){
  try{
  ...
  }catch(e){console.warn("Audit write failed",safeError(e));}
}
...
async function undoLastAuditAction(env,chatId){
  ...
  }catch(e){console.error("Undo failed",e);return `⚠️ مقدرتش أرجع العملية: ${safeError(e)}`;}
}
...
async function verifyCreatedScheduleBatch(env,chatId,one,rules){
  ...
  if(missing.length){
    ...
    throw new Error(`Verifier رفض العملية لأن الحفظ ناقص: ${missing.join(", ")}`);
  }
}
...
async function addShoppingItems(env,chatId,titles){
  const list=await getDefaultShoppingList(env,chatId,true);const now=new Date().toISOString();const ids=[];const added=[];
  ...
  for(const title0 of titles.slice(0,30)){
    ...
    const res=await env.DB.prepare(`INSERT INTO smart_list_items(list_id,chat_id,title,normalized_title,status,position,created_at,updated_at) VALUES (?,?,?,?, 'pending',?,?,?)`).bind(Number(list.id),chatId,title,n,++pos,now,now).run();ids.push(Number(res?.meta?.last_row_id||0));added.push(title);
  }
...
async function showShoppingHub(env,chatId,messageId=null){
  ...
  return editOrSend(env,chatId,messageId,`🛒 المشتريات\n\nمتبقي: ${pending}\nتم شراؤه: ${bought}\n\nتقدر تضيف أصناف بالكلام الطبيعي: «ضيف لبن وبيض للمشتريات».`,{inline_keyboard:[[{text:"🛒 افتح To‑Do",callback_data:"shop:open"}],[{text:"👀 عرض القائمة",callback_data:"shop:view"}],[{text:"↩️ لوحة التحكم",callback_data:"panel:home"}]]});
}
...
async function handleV10DirectCommands(env,chatId,text,{fromVoice=false}={}){
  ...
  if(m){const items=splitShoppingItems(m[1]);const r=await addShoppingItems(env,chatId,items);await sendText(env,chatId,r.added.length?`🛒 ضفت ${r.added.length} للمشتريات: ${r.added.join("، ")}`:"الأصناف دي موجودة بالفعل في المشتريات.");return true;}
...
async function transcribeTelegramVoice(env,voice){
  ...
  if(!info?.ok||!info?.result?.file_path)throw new Error("مقدرتش أحمل الفويس من Telegram.");
  ...
  // المسار الأساسي: نفس OmniAI Engine الموجود بالفعل في Super Agent.
  ...
  if(!key)throw new Error("محرك OmniAI لم ينجح في تفريغ الفويس. تأكد إن Groq/مزود Audio Transcription مفعّل داخل OmniAI.");
...
async function selfTestEndpoint(request,env){const url=new URL(request.url);const key=url.searchParams.get("key")||"";if(!env.SETUP_KEY||key!==env.SETUP_KEY)return json({ok:false,error:"Unauthorized"},401);return json({version:V10_VERSION,...runV10SelfTests()});}
