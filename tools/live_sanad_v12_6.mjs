...
id=await postText('خلي أبعت التقرير بعد نهاية اجتماع الفريق بنص ساعة');row=await wait(id);let dep=q(`SELECT * FROM sanad_dependencies WHERE chat_id='${esc(CHAT)}' AND active=1 ORDER BY id DESC LIMIT 1`)[0];pass('after-end dependency stored',dep?.relation==='after_end'&&Number(dep.offset_minutes)===30,JSON.stringify(dep));
...
id=await postText('حرّك اجتماع الفريق نص ساعة لقدام');row=await wait(id);b=q(`SELECT local_time FROM sanad_reminders WHERE chat_id='${esc(CHAT)}' AND id=${Number(b.id)}`)[0];pass('after-end dependency propagation',b?.local_time==='21:30',JSON.stringify(b));
