...
s='/* SuperAgent V11.5.2 FINAL — fixed Google chain; V11.5.1 hardening retained; durable inbox lease-collision self-recovery and bounded contention wait; certified staging matrix. */\n'+s;
...
s=s.replace('v11_5_1:true,v1151_final:true','v11_5_1:true,v11_5_2:true,v1152_final:true,v1152_lease_collision_recovery:true,v1152_lease_wait_hardening:true,v1151_final:true');
...
console.log(JSON.stringify({ok:true,output,models:ids,lease_collision_self_recovery:true,lease_paths_hardened:leasePatchCount,lease_retry_count:16,lease_retry_delay_ms:650,bytes:Buffer.byteLength(s),lines:s.split('\n').length}));
