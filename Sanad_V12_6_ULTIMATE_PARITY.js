/* Sanad V12.6 — Full-Life single-brain personal operating agent for Cloudflare Workers + D1 + Telegram.
...
const NAME = "سند — Sanad V12.6 Ultimate Parity";
...
function brainSystemPrompt(context) {
  return `أنت "سند" Sanad V12.6، مدير حياة شخصي Agent متعدد الأدوات. تتصرف كطقم سكرتارية واحد بعقل موحد.
...
async function runAgent(env,{chatId,text,user,operationId}) {
  ...
  if(complex&&!forcedSteps&&Date.now()<deadline-2500){
    try{const critic=await callBrainJson(env,`أنت مراجع خطط سند V12.6. راجع الخطة التالية مقابل طلب المستخدم والحالة. أصلح فقط الأخطاء: الأدوات الناقصة، IDs الخاطئة، الترتيب، أو خطوة قد تسبب false-success. لو خطوة تعتمد على نتيجة خطوة قبلها استخدم $step:N.field مثل $step:1.id، وتأكد أن كل جزء صريح من طلب المستخدم له خطوة تنفيذ فعلية. لا تضف خطوات بلا داع. أرجع JSON فقط {"steps":[...]}.\nطلب: ${text}\nالخطة: ${JSON.stringify(steps)}\nالحالة: ${JSON.stringify(context.state).slice(0,14000)}\nالأدوات: ${JSON.stringify(TOOL_SPECS)}`,text,deadline);if(Array.isArray(critic?.steps)&&critic.steps.length)steps=critic.steps.slice(0,MAX_AGENT_STEPS);}catch{}
  }
  ...
  const composer=`أنت سند V12.6. اكتب رد مصري طبيعي، مختصر ومفيد، بناءً فقط على observations. أي mutation هنا تم التحقق منه بالفعل. اذكر النتيجة المهمة لا تفاصيل النظام. لو المستخدم طلب أكتر من حاجة لخّصهم بوضوح.\nطلب: ${text}\nالنتائج: ${JSON.stringify(observations).slice(0,20000)}`;
...
async function selftest(request,env){
  ...
  add("version",VERSION==="12.6.0",VERSION);
